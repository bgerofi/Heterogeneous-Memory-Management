...
def parse_arguments():
	...
	parser.add_argument('--page-shift', action='store', type=int, default=12)
	parser.add_argument('--start-addr', action='store', default="0x0")
	parser.add_argument('--end-addr', action='store', default="{}".format(sys.maxsize))
	parser.add_argument('--start-time', action='store', type=int, default=0)
	parser.add_argument('--end-time', action='store', type=int, default=sys.maxsize)
		...
		while True:
			...
			if state == mmap:
				...
				if (elem == watermark):
					...
					#print("reading {} addresses for ts: {}, elapsed: {}".format(nelem, ts, (int(ts) - ts_start)/1400000000))
					ts_prev = int(ts)
...
def pebsData2heatmap(records, page_shift, range_size, start_i, end_i, start_addr, end_addr):
	maxaddr = 0
	minaddr = 0xffffffffffffffff
	ind = -1
	...
	for ts, record in enumerate(records):
		...
		for i, addr in enumerate(record):
			...
			if heatmap.get(paddr) == None:
				heatmap[paddr] = 1
			else:
				heatmap[paddr] += 1

			if heatmap[paddr] > max_heat_cnt:
				max_heat_paddr = paddr
				max_heat_cnt = heatmap[paddr]
	...
	hmap = np.zeros((nr_addresses, len(heatmaps)))
	...
	for ind, heatmap in enumerate(heatmaps):
		for paddr in heatmap.keys():
			...
			if global_heatmap.get(paddr) == None:
				global_heatmap[paddr] = 1
			else:
				global_heatmap[paddr] += 1
	...
	pages_reported = 0
	while addr < maxaddr:
		if global_heatmap.get(addr) == None:
			pages_missed += 1
		else:
			pages_reported += 1
		addr += (1 << page_shift)
	
	print("pages_missed: {}, pages_reported: {}".format(pages_missed, pages_reported))
...
if __name__ == '__main__':
	...
	majorlocator = 1
	minorlocator = 1
	if args.page_shift < 20:
		majorlocator = 1 << (20 - args.page_shift)
		minorlocator = majorlocator / 4

	my_cmap = mpl.colors.LinearSegmentedColormap.from_list('my_colormap', colors=["white", "red", "pink", "cyan", "yellow"])
	...
	hmap, minaddr = pebsData2heatmap(records, args.page_shift, range_size, args.start_time, args.end_time, int(args.start_addr, 0), int(args.end_addr, 0))
	fig, ax = plt.subplots()
	#cax = ax.imshow(hmap, cmap='hot', interpolation='none')
	cax = ax.imshow(hmap, cmap=my_cmap, interpolation='none')
	axes = plt.gca()
	fmt = AddressFormatter(minaddr, args.page_shift)
	ax.get_yaxis().set_major_formatter(fmt)
	if args.end_time < sys.maxsize:
		print("elapsed inspected cycles: {}".format(tss[args.end_time] - tss[args.start_time]))
	print("elapsed total cycles: {}".format(tss[-1] - tss[0]))

	ax.get_yaxis().set_major_locator(ticker.MultipleLocator(majorlocator))
	ax.get_yaxis().set_minor_locator(ticker.MultipleLocator(minorlocator))

	plt.xlabel("Sample set ID")
	plt.ylabel("Virtual Address")
	fig.tight_layout()
	#ax.set_aspect(1.5)
	plt.subplots_adjust(wspace=0.0, hspace=0.0, left=-0.3, right=1.4, top=0.95, bottom=0.1)
	plt.show()
